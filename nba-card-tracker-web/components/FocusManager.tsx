...
import type { FocusRow, FocusType } from "@/lib/types";
...
/**
 * Creation + gestion (renommage, activation, suppression) des focus.
 * Vivait auparavant sur /profil ; deplace ici pour plus de logique : on cree
 * et on gere ses focus au meme endroit qu'on les consulte.
 */
export default function FocusManager({ focuses, options, focusProgress }: FocusManagerProps) {
  ...
  return (
    <section className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
      <h2 className="text-sm font-bold">Gérer mes focus</h2>
      ...
      {error ? (
        <p role="alert" className="mt-3 rounded-lg bg-red-100 px-3 py-2 text-sm text-red-800 dark:bg-red-950/60 dark:text-red-200">
          {error}
        </p>
      ) : null}
      {saved ? (
        <p role="status" className="mt-3 rounded-lg bg-emerald-100 px-3 py-2 text-sm text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-200">
          {saved}
        </p>
      ) : null}
    </section>
